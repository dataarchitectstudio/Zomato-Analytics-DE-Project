...
def generate_menu_items(
    ...
    item_categories = [
        ...
        "Desserts", "Beverages", "Combos", "Sides", "Soups", "Salads",
    ]
...
def generate_orders(
    ...
    for i in range(volume.num_orders):
        ...
        orders.append({
            ...
            "payment_status": "Completed" if status in ("Delivered",) else random.choice(["Pending", "Completed", "Failed"]),
...
def generate_deliveries(
    ...
    deliverable_orders = [
        o for o in orders
        if o["order_status"] in ("Out for Delivery", "Delivered")
    ]
    deliveries = []
...
def generate_reviews(
    ...
    for i in range(min(volume.num_reviews, len(delivered_orders))):
        ...
        review_ts = datetime.fromisoformat(order["order_placed_at"]) + timedelta(
        ...
        reviews.append({
            ...
            "rating": rating,
            "review_text": random.choice(review_sentiments) if random.random() < 0.7 else fake.paragraph(nb_sentences=2),
